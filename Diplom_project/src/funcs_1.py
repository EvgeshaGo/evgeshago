...
def get_metrics(
        y_true_:pd.Series, y_pred_:pd.Series, 
    ) -> None:
    ...
    rmse_score = np.sqrt(mse_score)  # Вычисляем RMSE вручную
    ...
    def get_metrics(
        y_true_: pd.Series,  #  Изменили тип на pd.Series
        y_pred_: pd.Series, # Изменили тип на pd.Series
) -> None:
        """ Функция, реализующая вычисление метрик MAPE, MSE, RMSE.
    ...
    # Выводим метрики
    print(f'MAPE: {mape_score:.2f} %')  # Убрали TOL, упростили формат
    print(f'MSE: {mse_score:.2e} $^2')  # Убрали TOL, упростили формат
    print(f'RMSE: {rmse_score:.2e} $') # Убрали TOL, упростили формат
